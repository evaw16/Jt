    ...
    <div id="sidebar">
      ...
      <div id="main-panel">
          ...
          <nav class="navbar navbar-default">
            <div class="container-fluid">
              <div class="navbar-header">
                ...
                <a class="navbar-brand text-size-24" href="#"><i class="fa fa-fw fa-bell"></i> Verifikasi Proposal</a>
              </div>
        ...
        <table class="table table-bordered" style="margin-left:110px; width:950px;">
          ...
          <tbody>
            <?php
            $sql="SELECT proposal.id_proposal, aktor.nama, aktor.nama_wisata, proposal.proposal,
                         proposal.tanggal, proposal.status
                  FROM proposal JOIN aktor ON proposal.id_pegawai = aktor.id_pegawai";
            $result = mysqli_query($con,$sql);
            while ($item = mysqli_fetch_array($result)) {
              ?>
              <tr>
                <td hidden><?=$item['id_proposal']?></td>
                <td style="text-align:center"><?=$item['nama']?></td>
                <td style="text-align:center"><?=$item['nama_wisata']?></td>
                <td style="text-align:center"><?=$item['proposal']?><enctype="multipart/form-data" method="POST"></td> 
                <td style="text-align:center"><?=$item['tanggal']?></td>
                <td style="text-align:center"><?=$item['status']?></td>
                <td>
                  <p style="text-align:center"><a href="terimaprop.php?id=<?php echo $item['id_proposal']?>" onclick="return confirm('Anda akan menerima proposal');"><button class="btn btn-success bold">Terima</button></a>
                <a href="tdkterimaprop.php?id=<?php echo $item['id_proposal']?>" onclick="return confirm('Anda akan menolak proposal?');"><button class="btn btn-warning bold">Tolak</button></a></p>
              </tr>
